...
class SimpleTrainer2d:
    ...
    def __init__(
        ...
        elif model_name == "TileEncoding":
            from gaussianimage_rs_v2 import GaussianImage_RS_MultiBook
            self.gaussian_model = GaussianImage_RS_MultiBook(loss_type="L2", opt_type="adan", num_points=self.num_points, H=self.H, W=self.W, BLOCK_H=BLOCK_H, BLOCK_W=BLOCK_W, 
                device=self.device, lr=args.lr, quantize=True).to(self.device)

        elif model_name == "Cluster":
            from gs_cluster import GaussianImage_RS_MultiBook
            self.gaussian_model = GaussianImage_RS_MultiBook(loss_type="L2", opt_type="adan", num_points=self.num_points, H=self.H, W=self.W, BLOCK_H=BLOCK_H, BLOCK_W=BLOCK_W, 
                device=self.device, lr=args.lr, quantize=True, num_clusters=16).to(self.device)
...
def main(argv):
    ...
    with open(f"quantize_{args.model_name}_log.csv", "a", newline="") as f:
        writer = csv.writer(f)
